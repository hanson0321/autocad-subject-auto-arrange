...
def set_aisle(empty_area,door):
    unusable_gridcell = []
    for polygon in empty_area:
        ### 紀錄empty座標位置(左、下、右、上)
        left, bottom, right, top = polygon.bounds
    ...
    for cell in door:
        if(cell['orientation'] == 'east'):
            unusable_gridcell.append({'type':'aisle', 'x':cell['location']-aisle, 'y':cell['range'][0], 'w':aisle, 'h':cell['range'][1]-cell['range'][0]})
        elif(cell['orientation'] == 'west'):
            unusable_gridcell.append({'type':'aisle', 'x':cell['location'], 'y':cell['range'][0], 'w':aisle, 'h':cell['range'][1]-cell['range'][0]})
        elif(cell['orientation'] == 'south'):
            unusable_gridcell.append({'type':'aisle', 'x':cell['range'][0], 'y':cell['location'], 'w':cell['range'][1]-cell['range'][0], 'h':aisle})
        else: ### cell['orientation'] == 'north'
            unusable_gridcell.append({'type':'aisle', 'x':cell['range'][0], 'y':cell['location']-aisle, 'w':cell['range'][1]-cell['range'][0], 'h':aisle})
    
    return unusable_gridcell
...
def feasible_area(doc):
    ...
    unusable_gridcell = set_aisle(rectangles_empty,door)
    def range_width(item):
        return item['range'][1] - item['range'][0]

# Find the dictionary with the widest range
    frontdoor_dict = max(door, key=range_width)

    if frontdoor_dict['orientation'] == 'north' or frontdoor_dict['orientation'] == 'south':
        frontdoor = LineString([(frontdoor_dict['range'][0],frontdoor_dict['location']),(frontdoor_dict['range'][1],frontdoor_dict['location'])])
    else:
        frontdoor = LineString([(frontdoor_dict['location'],frontdoor_dict['range'][0]),(frontdoor_dict['location'],frontdoor_dict['range'][1])])
    unusable_gridcell_dict = {i: item for i, item in enumerate(unusable_gridcell)}
    return unusable_gridcell, unusable_gridcell_dict,min_x,max_x,min_y,max_y, poly_feasible,wall, door, frontdoor

def draw_line(data, color):
    for line in data:
        if(line['orientation'] == 'east' or line['orientation'] == 'west'):
            x1,y1 = line['location'],line['range'][0]
            x2,y2 = line['location'],line['range'][1]
        else:
            x1,y1 = line['range'][0],line['location']
            x2,y2 = line['range'][1],line['location']
        ax.plot([x1, x2], [y1, y2],color=color)
            
if __name__ == "__main__":
    doc = '/Users/lilianliao/Downloads/layout_part2改/revise.dxf'
    unusable_gridcell,unusable_gridcell_dict, min_x, max_x, min_y, max_y, poly_feasible, wall, door, frontdoor = feasible_area(doc)
    ...
    ### 繪製 empty area
    for id, cell in unusable_gridcell_dict.items():
        x,y = cell['x'], cell['y']
    ...
    ### 存檔
    plt.savefig('get_feasible_area.png', dpi=300, bbox_inches='tight')
