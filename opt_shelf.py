...
def shelf_opt(shelf_area, unusable_gridcell1, counter_placement, door_side, shelf_params, priority_shelves, gap_between_shelves):
    ...
    for key, values in shelf_placement.items():
        shelf_placement[key]['type'] = 'objects'
        if(values['w']>values['h']):
            shelf_placement[key]['orientation'] = 'east-west'
        else:
            shelf_placement[key]['orientation'] = 'south-north'
